...
- TODO
  - the gui to prompt the user on install
    - non ajax install got usual GUI for the user
    - good as it is not confused
  - determine the strategy as of when to download the data..
    - ask for the user to OK then download
      - PRO: user bandwidth is used only after his explicit authorisation
      - CON: the installation is longuer as the user have to wait for the
        whole download of the update
    - download and then ask the user to OK
      - PRO: installation in a snap (only local file moving)
      - CON: update may be downloaded while the user do need all his bandwidth
  - when to query the server for update
    - every 10sec :)
    - at random in the first hour after boot ?
- Docs about this topic
  - http://www.robertnyman.com/2008/06/09/how-to-update-adobe-air-applications-automatically-with-javascript-airupdaterjs/
  - http://code.google.com/p/airupdater/downloads/list
  - http://www.insideria.com/2008/03/air-api-performing-updates-in-1.html
  - http://www.insideria.com/2008/02/air-api---intro-to-the-updater.html
- PRO: this updater is interesting as it is fully js (aka no .swf i dont understand)
- CON: i need to do it all :) aka the GUI and all 
	
------------------
Originally written by Robert Nyman, http://www.robertnyman.com MIT License
...
var AIRUpdater = function () {
	...
	var applicationName	= "urfastr-player.air";
	...
	*/
	var latestVersionCheckUrl = "http://urfastr.net/static/player/widget/adobe_air2/bin/version.xml";
	...
	*/	
	var updaterUrl	= "http://urfastr.net/static/player/widget/adobe_air2/bin/urfastr-player-";
	var stream	= null;
	var updateFile	= null;
	...
	var getApplicationVersion = function () {
		...
		air.trace("current version is " + applicationVersion);
	};
		...
		*/	
		...
		XMLHttp.onreadystatechange = function () {
			if (XMLHttp.readyState === 4) {
				air.trace('got version.xml file');
				...
				*/
				if (typeof releaseNotesNode === "object" && releaseNotesNode.firstChild) {
					...
					air.trace('releaseNotes=' + releaseNotesText);
				}
		...
		air.trace('start getting version.xml file');
	...
	var initUpdateApplication = function () {
		...
		*/
		air.trace('init update appliction');
		...
		*/
		...
		stream.load( new air.URLRequest("http://urfastr.net/static/player/widget/adobe_air2/bin/urfastr-player.air"));
	};
	...
	updateApplication = function () {
		air.trace('do update application');
	...
	var installUpdate = function () {
		...
		air.trace('only need to upadte now');
	};
