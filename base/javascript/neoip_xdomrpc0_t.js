...
// TODO to fix. when apps_detect_t delete xdomrpc_t into its own callback
// - the following code is executed and it makes IE crash (tested on IE6sp2 and IE7)
// - likely because i remove the element _post while running it
// - POSSIBLE SOLUTION: have this delete done in a timeout 0 
...
if( 1 ){	// TODO to remove ? old version which is synchronous
	...
	//   - likely because i remove the element _post while running it
	var async_delete_cb	= function(p_htmlid_root){
		var	htmlid_root	= p_htmlid_root;
		return	function() {
			try {	div_elem.removeChild(document.getElementById(htmlid_root + "_pre"));
			}catch(e){}
			try {	div_elem.removeChild(document.getElementById(htmlid_root + "_call"));
	...
	// start the timeout
	setTimeout(async_delete_cb(htmlid_root), 1*1000);
}
}
 ...
 */
function neoip_xdomrpc_cb_callback_from_server(obj_id, reply)
{
	// log to debug
