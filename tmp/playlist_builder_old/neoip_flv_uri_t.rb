...
def self.to_external_flv_mdata_file(flv_uri)
	...
	extmdata_path	= "../../player/cache/flv_mdata_xml/#{basename_full}";

	# build the extmdata_uri
	extmdata_uri	= "#{global_extmdata_uri_base()}/#{basename_full}";
...
def self.to_track_jspf(flv_uri, track_title)
	...
	oload	= {};
	oload['mod']				= "flv";
	oload['http_peersrc_uri']		= global_http_peersrc_uri();
