...
query_arg	= {	"type"		=>	"users",
			"username"	=>	"seeso",
			"subtype"	=>	"uploads",
			"query_param"	=> {	"start-index"	=> 1,
						"max-results"	=> 50
					},
			"category_tag"	=> []
		};
		
query_arg	= {	"type"		=>	"videos",
			"category_tag"	=>	[ "Howto" ]
		};
result		= Youtube_gdata_t.query(query_arg);

#pp result

puts "total_result=#{result['totalResults']}"

result['entry'].each { |entry|
