...
package com.mattism.http.xmlrpc
{
	...
	public class ConnectionImpl extends EventDispatcher implements Connection {
		...
		public function ConnectionImpl( url:String ) {
			...
			// jme- added to handle network error
			this._response.addEventListener(IOErrorEvent.IO_ERROR, this._onIoError);
		...
		public function destructor()	:void
		{
			if( this._response ){
				...
				this._response.removeEventListener	(IOErrorEvent.IO_ERROR, this._onIoError);
				this._response	= null;
			}
		}
		...
		private function _onIoError( evt:IOErrorEvent ):void {
			// log to debug
			console.info("error in the call+++++++ evt=" + evt);
			// create a 'fake' fault for the notified to know
			this._fault	= new MethodFaultImpl({	  faultCode	: -1
								, faultString	: "ioerror"});
