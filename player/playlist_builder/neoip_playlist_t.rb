...
def initialize(playlist_id = nil, playlist_title = nil)
	...
	if( playlist_id )
		...
		@jspf['meta']['reload_delay']	= 30*1000;	# TODO this is a bit short...
		@jspf['track']		= []
	end
end
...
# create a playlist from a json string (supposed to be in jspf - json version of xspf)
...
# convert the current Playlist_t to json format
def to_json()
	return JSON.pretty_generate({"playlist" => @jspf});
end
...
def self.from_file(filename)
	...
	# build a playlist with the file_data
	playlist	= self.from_json(file_data)
	# return the just built playlist
	return playlist
end
